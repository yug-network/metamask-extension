 ...
 *
 * Our overall goal we want to partition the graph into different sections. We
 * want to find the leaves of the graph — that is, files that depend on no other
 * files — then the dependents of the leaves — those files that depend on the
 * leaves — then the dependents of the dependents, etc. We can derive this
 * information by traversing the graph, and for each module we encounter,
 * recording the number of modules it takes to reach that module. We call this
 * number the "level".
 *
 ...
 * modules may be visited multiple times n the course of traversing connections
